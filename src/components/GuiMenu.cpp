...
void GuiMenu::populateList()
{
  ...
  mList->addObject("Setup controllers", "rm .glint-es/es_input.cfg && pkill glint-es && glint-es/glint-es", 0xFFFFFFFF);
...
{
	Renderer::drawRect(Renderer::getScreenWidth() * 0.25, 0, Renderer::getScreenWidth() * 0.5, Renderer::getScreenHeight(), 0x999999FF);
}
